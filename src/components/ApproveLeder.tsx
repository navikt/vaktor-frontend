...
const mapAudit = (audit: Audit[]) => {
  return audit.map((audit: Audit, index) => (
    <div key={audit.id}>
      <ReadMore
        header={audit.timestamp.slice(0, -10).replace("T", " ")}
        size="small"
        style={
...
const mapApproveStatus = (status: number) => {
  ...
  return (
    <Table.DataCell
      style={{ backgroundColor: statusColor, maxWidth: "150", minWidth: "150" }}
    >
      {statusText}
    </Table.DataCell>
  );
  ...
  const mapVakter = (vaktliste: Schedules[]) =>
    vaktliste.map((vakter: Schedules, i: number) => (
      ...
      <Table.Row key={i}>
        ...
        {["personalleder", "leveranseleder"].includes(currentUser!.role) && (
          <Table.DataCell
            scope="row"
    ...
    <>
      <div className="min-h-96" style={{ display: "flex" }}>
        ...
        <div style={{ width: "200px", marginLeft: "30px" }}>
          <Select
            label="Velg handling"
            onChange={(e) => setSearchFilterAction(Number(e.target.value))}
          >
